...
%                 slope = s(idxk);                                 % scalar
%                 gradient = gradient + slope * (a_row + Xi_current);            % 1 x n
%             end
%             gradient_penalty1 = M*max(0, x_est-1);
%             gradient_penalty2 = -M*max(0, -1-x_est);
%             gradient_sum = gradient + gradient_penalty1 + gradient_penalty2;
%             if max(gradient_sum)>max_Lip
%                 max_Lip = max(gradient_sum);
%             end
%             gamma = 1/max_Lip;
%         end
        ...
        for i_rep = 1:replications
            ...
            for i = 1:iter
                ...
                obj_val_i = obj_val_i/N + 0.5*lambda*norm(x_temp,1) ...
                ...
                z = x_temp - gamma * gradient_sum;

                % proximal step (soft thresholding)
                x_temp = sign(z) .* max(abs(z) - gamma * lambda*0.5, 0);
...
outname = sprintf('experiment_results_replications_lasso_%s.mat', datestr(now,'yyyymmdd_HHMMSS'));
