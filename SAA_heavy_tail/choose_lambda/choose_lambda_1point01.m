...
temp_obj = rand(N, n); %put this outside of for loop to keep the same validation set
...
%         end
        ...
        %display(norm(gradient_penalty1))
        ...
        if ~isreal(vec) || ~isreal(obj_val_i)
            fprintf('Complex value detected at iter %d (n=%d, lambda=%.3f). Stopping early.\n', ...
                 i, n, lambda);
            i_last = i;
            break;
        end
...
save('../lambda_1point01.mat', 'lambda');
